...
const arrayFromAsyncIterator = async <T>(i: AsyncGenerator<T, void, unknown>): Promise<T[]> => {
  const r: T[] = []
...
const useCache = async <T>(name: string, f: () => T): Promise<T> => {
  ...
  const res = await f()
  await fs.promises.writeFile(filename, JSON.stringify(res), { encoding: 'utf-8' })
  return res
}
...
const fetchRegisteredCources = async (session: Fetch, menu: Menu) => {
  ...
  const registered = Array.from(regPageFragment.querySelectorAll('.rishu-koma-inner'))
    ...
    .map(t => t.split("\n").map(l => l.trim()))
    .map(v => {
      if (v.length !== 3) throw new Error(`3行に分割できていません: ${JSON.stringify(v)}`)
      return {
        timetableCode: v[0],
        course: v[1],
        lecturers: v[2],
  ...
  return registered.map(({ timetableCode, ...rest }) => {
    const digest = digests.find(({ options }) => options.jikanwaricd === timetableCode)
    if (!digest) throw new Error(`履修登録されている時間割コード ${timetableCode} に該当するシラバス要約が見つかりません。キャッシュを利用しないオプションを付けた上で再度実行してみてください`)
    return {
      refer: digest,
      timetableCode,
      ...rest,
...
const main = async () => {
  ...
  for (const cource of registeredCources) {
    ...
    const jikanwariInputFormInput = convertFormElementsToPlainKeyValueObject(jikanwariInputForm, {})
    jikanwariInputFormInput["jikanwariShozokuCodeForKettei"] = cource.faculityOfCourse
    jikanwariInputFormInput["jikanwaricd"] = cource.timetableCode

    const syllabusPage = await session(resolve(syllabusSearchPage.url, jikanwariInputForm.action), {
