...
// fs ni suru
const readSyllabus = (): Promise<SYLLABUS> => Promise.resolve(SYLLABUS_SCHEMA.transformOrThrow(require('../../syllabus.json')))
...
const CODE_PATH = [['講義概要/Course Information', '科目基礎情報/General Information'], '科目番号/Code'] as [[string, string], string]
const SYLLABUS_SCHEMA = $.array($.obj({
  digest: $.obj({
    学期: $.literal('前学期', '後学期'),
    開講: $.literal('前学期', '後学期'),
...
const findPath = <P extends Path>(p: P, t: SYLLABUS[0]['contentTree']) => {
  const titlePath = p[0]
  const findContent = (c: ContentNode[], p: string[]): ContentNode | undefined => {
...
const calculateCalendarFromJigen = (j: string[]): Cal => {
  ...
  const fromTimeString = (s: string): Time => {
    ...
    if (times.length !== 2 || times.some(v => Number.isNaN(v))) throw new Error()
  ...
  const schedule = j.map((jikanwari: string) => {
    // 月7 など
    const s = timeSchedule[jikanwari.slice(1).slice(0,1)]
    if (!s) throw new Error(`'${jikanwari}' の時限の時間割が見つかりませんでした`)
    const dow = dayOfWeek.findIndex(v => v == jikanwari.slice(0,1))
    if (dow === -1) throw new Error(`'${jikanwari}' の曜日が見つかりませんでした`)
    return {
      dayOfWeek: dow,
      ...s
    }
  }).reduce((s, cur) => {
    if (s.dayOfWeek !== cur.dayOfWeek) throw new Error('別の曜日が指定されています')
...
const main = async () => {
  ...
  const { data: { id: calendarId } } = await calendar.calendars.insert({
    requestBody: {
      summary: 'シラバスカレンダー',
      timeZone: 'Asia/Tokyo'
    },
  ...
  const persisted: { calendarId: string, eventId: string, courseId: string, timetableId: string }[] = []
  ...
  for (const course of courses) {
    ...
    const start = setTime(new Date(firstBaseDay), course.calendar.start, course.calendar.dayOfWeek)
    const end = setTime(new Date(firstBaseDay), course.calendar.end, course.calendar.dayOfWeek)
    const event = {
      summary: `${course.科目}`,
      start: {
